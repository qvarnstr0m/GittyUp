...
getLatestEvents(GittyupConfig.username, GittyupConfig.numEvents).then(
  (events) => {
    ...
    events.forEach((event) => {
      ...
      li.innerHTML =
        GittyupConfig.includeDateTime === true
          ? `${eventType} ${eventDescription} <a href="https://www.github.com/${event.repo.name}" 
    target="_blank" rel="noopener noreferrer">${event.repo.name}</a> at ${createdAt}`
